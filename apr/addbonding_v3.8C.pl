...
# v3.20091023 clhuang fix routing blockage top metel info. in io_A.list
# v3.5 20101123 nschang fix bugs, re-coding add bonding info
#use FileHandle;

&print_usage if(@ARGV == 0);

my $iolist="io_C.list"; #default name
for (my $i=0;$i<@ARGV;$i++){
    if($ARGV[$i] =~ /-io/){
        $i++;
        $iolist=$ARGV[$i];
...
sub readIOPadList(){
    ...
    if($version != 3.6){
        print("\nERROR, io_A.list VERSION inconsistence. Required version: 3.6\n\n");
        exit;
    }
...
** named io_A.list. This file list all the io pads in the IO library.
